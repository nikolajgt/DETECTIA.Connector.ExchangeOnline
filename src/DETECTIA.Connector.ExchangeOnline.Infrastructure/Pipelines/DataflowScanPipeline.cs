...
public static class DataflowScanPipeline
{
    ...
    // }
     ...
     public static async Task RunAsync<TInput, TEntity, TMatch>(
        Func<long, CancellationToken, Task<List<TInput>?>> fetchPageAsync,
        Func<TInput, CancellationToken, Task<(TEntity entity, IEnumerable<TMatch> matches)>> expandAsync,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var fanOutBlock = new TransformBlock<TInput, (TEntity entity, IEnumerable<TMatch> matches)>(
        ...
        var batchBlock = new BatchBlock<(TEntity entity, IEnumerable<TMatch> matches)>(persistBatchSize);

        var flattenBlock = new TransformBlock<(TEntity entity, IEnumerable<TMatch> matches)[], (IEnumerable<TEntity>, IEnumerable<TMatch>)>(
            batch =>
            {
                var allEntities = batch.Select(item => item.entity).ToList();
                var allMatches = batch.SelectMany(item => item.matches).ToList();
